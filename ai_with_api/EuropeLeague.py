...
class FootballDataCache:
    ...
    def get_team_domestic_league(self, team_id):
        """Get the domestic league ID for a Europa League team"""
        ...
        if response.status_code == 200:
            ...
            if "teams" in data and data["teams"]:
                ...
                return team_data.get("idLeague")
        return None

    def calculate_europa_league_stats(self):
        """Calculate stats for Europa League teams using domestic performance"""
        europa_teams = self.cache.get_teams()  # Already fetched Europa League teams
        team_stats = {}

        for team in europa_teams:
            team_id = team["idTeam"]
            team_name = team["strTeam"]

            # Get domestic league ID
            domestic_league_id = self.get_team_domestic_league(team_id)
            if not domestic_league_id:
                print(f"Couldn't find domestic league for {team_name}")
                continue

            # Create a temporary cache for this domestic league
            domestic_cache = FootballDataCache(self.api_key, domestic_league_id,
                                               f"cache_domestic_{domestic_league_id}")

            # Get domestic matches
            domestic_matches = []
            current_year = datetime.now().year
            seasons = [f"{current_year - 1}-{current_year}"]

            for season in seasons:
                matches = domestic_cache.get_season_matches(season)
                domestic_matches.extend([m for m in matches
                                         if m["strStatus"] == "Match Finished" and
                                         (m["idHomeTeam"] == team_id or
                                          m["idAwayTeam"] == team_id)])

            # Calculate stats based on domestic performance
            if domestic_matches:
                team_stats[team_id] = self.calculate_team_stats(team_id, domestic_matches)
                print(f"Calculated domestic stats for {team_name} from {len(domestic_matches)} matches")
